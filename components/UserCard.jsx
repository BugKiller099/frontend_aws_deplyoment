...
const UserCard = ({ user, isPreview = false }) => {
  ...
  return (
    <div
      ref={cardRef}
      className={`relative ${isPreview ? "w-full max-w-sm" : "w-full max-w-md"} h-[600px] mx-auto select-none`}
    ...
    >
      <div
        ...
        style={{
          transform: `translateX(${swipePosition}px) rotate(${swipePosition / 30}deg) scale(${1 - Math.abs(swipePosition) / (2 * window.innerWidth)})`,
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 text-white z-10">
          <div className="flex items-end justify-between">
            ...
            <button
              ...
              onClick={toggleBio}
            >
              {showFullBio ? <ChevronDown className="w-6 h-6 text-white" /> : <Info className="w-6 h-6 text-white" />}
            </button>
          </div>
          ...
          {about && (
            <div
              className={`mt-4 bg-black/30 backdrop-blur-sm p-3 rounded-xl transition-all duration-300 ${
                showFullBio ? "max-h-32 overflow-y-auto" : "max-h-16 overflow-hidden"
              }`}
            >
              <p className={`text-sm text-white/90 ${showFullBio ? "" : "line-clamp-2"}`}>{about}</p>
            </div>
          )}
        </div>
      </div>
