...
const Chat = () => {
  ...
  const socketRef = useChatSocket({
    ...
    onUserStatusChange: ({ userId: changedUserId, status }) => {
      if (changedUserId === targetUserId) {
        setTargetUserInfo((prev) => prev ? { ...prev, onlineStatus: status } : prev)
      }
    },
    onTyping: () => {
      setIsTyping(true)
          ...
          <div className="flex items-center flex-1">
            <div className="relative">
              ...
              <div 
                className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-white dark:border-gray-800 ${
                  targetUserInfo.onlineStatus === "online" ? "bg-green-500" : "bg-gray-400"
                }`}
              ></div>
            </div>
            ...
            <div className="ml-3">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">
                {isTyping ? "typing..." : targetUserInfo.onlineStatus === "online" ? "Online" : "Offline"}
              </p>
            </div>
          </div>
        )}
                ...
                {message.sender !== "self" && (
                  <div className="w-8 h-8 rounded-full overflow-hidden flex-shrink-0">
                    <img
                      ...
                      onError={(e) => {
                        console.log("Message avatar load error, using placeholder")
                        e.target.onerror = null // Prevent infinite error loop
                        e.target.src = PLACEHOLDER_AVATAR
                      }}
                    />
